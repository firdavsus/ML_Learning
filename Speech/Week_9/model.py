...
class MNISTEncoderDecoder(L.LightningModule):
    ...
    def training_step_with_quantizer(self, pictures_batch):
        ...
        quantized_ste = self.quantizer(encoded)
        predicted = self.decoder(quantized_ste)

        indices = self.quantizer.encode(encoded)
        quantized_raw = self.quantizer.decode(indices)

        reconstr_loss = self.reconstr_loss_fn(predicted, pictures_batch)
        vq_loss = self.vq_loss_fn(encoded, quantized_raw)

        loss = reconstr_loss + vq_loss

        if self._trainer is not None:
            self.log("train/reconstr_loss", reconstr_loss.item(), prog_bar=False)
            self.log("train/vq_loss", vq_loss.item(), prog_bar=False)
